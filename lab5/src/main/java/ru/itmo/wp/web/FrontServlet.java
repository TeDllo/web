...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Map<Class<?>, Object> paramMap = Map.of(
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            ...
            Arrays.sort(methods, Comparator.comparing(Method::hashCode));

            for (Method declaredMethod: methods) {
                boolean correctName = declaredMethod.getName().equals(route.getAction());
                boolean correctTypes = Arrays.stream(declaredMethod.getParameterTypes())
                        .allMatch(paramMap.keySet()::contains);

                if (correctName && correctTypes) {
        ...
        for (Class<?> paramClass : paramMap.keySet()) {
            if (Arrays.asList(method.getParameterTypes()).contains(paramClass)) {
                args.add(paramMap.get(paramClass));
            }
        }
        ...
        String defaultName = pageClass.getSimpleName();
        ...
        Template template;
        try {
            template = newTemplate(fullName + ".ftlh");
        } catch (ServletException e) {
            template = newTemplate(defaultName + ".ftlh");
        }
